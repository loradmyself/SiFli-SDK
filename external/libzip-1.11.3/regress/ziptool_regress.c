...
#define DISPATCH_REGRESS \
    ...
    {"libzip_fopen", 1, "name", "open archive entry", regress_fopen}, \
    {"libzip_fread", 2, "file_index length", "read from fopened file and print", regress_fread}, \
    {"libzip_fseek", 3, "file_index offset whence", "seek in fopened file", regress_fseek}, \
...
regress_fread(char *argv[]) {
    ...
    while (length > 0) {
        ...
        if (libzip_fwrite(buf, (size_t)n, 1, stdout) != 1) {
...
read_to_memory(const char *archive, int flags, zip_error_t *error, zip_source_t **srcp) {
    ...
    if ((fp = libzip_fopen(archive, "rb")) == NULL) {
        if (errno == ENOENT) {
        ...
        if (fstat(libzip_fileno(fp), &st) < 0) {
            libzip_fclose(fp);
            zip_error_set(error, ZIP_ER_OPEN, errno);
            return NULL;
        }
        if (fragment_size == 0) {
            char *buf;
            if ((buf = zip_alloc((size_t)st.st_size)) == NULL) {
                libzip_fclose(fp);
                zip_error_set(error, ZIP_ER_MEMORY, 0);
                return NULL;
            }
            if (libzip_fread(buf, (size_t)st.st_size, 1, fp) < 1) {
                zip_free(buf);
                libzip_fclose(fp);
            ...
            if ((fragments = zip_alloc(sizeof(fragments[0]) * nfragments)) == NULL) {
                libzip_fclose(fp);
                zip_error_set(error, ZIP_ER_MEMORY, 0);
                return NULL;
            }
            for (i = 0; i < nfragments; i++) {
                ...
                if ((fragments[i].data = zip_alloc(left)) == NULL) {
#ifndef __clang_analyzer__
                    /* fragments is initialized up to i - 1*/
                    while (--i > 0) {
                        zip_free(fragments[i].data);
                    }
#endif
                    zip_free(fragments);
                    libzip_fclose(fp);
                ...
                if (libzip_fread(fragments[i].data, left, 1, fp) < 1) {
#ifndef __clang_analyzer__
                    /* fragments is initialized up to i - 1*/
                    while (--i > 0) {
                        zip_free(fragments[i].data);
...
source_nul(zip_t *zs, zip_uint64_t length) {
    ...
    if ((ctx = (source_nul_t *)zip_alloc(sizeof(*ctx))) == NULL) {
...
write_memory_src_to_file(const char *archive, zip_source_t *src) {
    ...
    if ((buf = zip_alloc(zst.size)) == NULL) {
        fprintf(stderr, "zip_alloc failed: %s\n", strerror(errno));
    ...
    zip_source_close(src);
    if ((fp = libzip_fopen(archive, "wb")) == NULL) {
        fprintf(stderr, "libzip_fopen failed: %s\n", strerror(errno));
        zip_free(buf);
        return -1;
    }
    if (libzip_fwrite(buf, zst.size, 1, fp) < 1) {
        fprintf(stderr, "libzip_fwrite failed: %s\n", strerror(errno));
        zip_free(buf);
        libzip_fclose(fp);
        return -1;
    }
    zip_free(buf);
    if (libzip_fclose(fp) != 0) {
        fprintf(stderr, "libzip_fclose failed: %s\n", strerror(errno));
        return -1;
    }
    return 0;
}
