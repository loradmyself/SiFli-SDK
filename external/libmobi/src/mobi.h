...
extern "C"
{
    ...
    */
    typedef struct {
        ...
        const MOBIData *m; /**< Source MOBI data for embedded low-memory lazy reconstruction */
    ...
    MOBI_EXPORT MOBI_RET mobi_reconstruct_part_data(MOBIRawml *rawml, const size_t uid, MOBIPart **part);

    MOBI_EXPORT MOBI_RET mobi_get_rawml(const MOBIData *m, char *text, size_t *len);
    MOBI_EXPORT MOBI_RET mobi_get_rawml_record(const MOBIData *m, size_t record_index, char *text, size_t *len);
    ...
    MOBI_EXPORT char * mobi_decode_set_img_src(const char *book_path, uint32_t uid);
    MOBI_EXPORT char * mobi_decode_set_cover_src(const char *book_path, const char *cover_name);
    MOBI_EXPORT void mobi_decode_free(void *ptr);
    MOBI_EXPORT int mobi_decoder_img_init(void);
    ...
    MOBI_EXPORT lv_res_t mobi_decode_get_img_info(const char *book_path, uint32_t uid, bool is_cover, lv_img_header_t *header);
