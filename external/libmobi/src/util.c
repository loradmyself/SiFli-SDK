...
/**
 @brief Decompress a single MOBI text record to caller supplied buffer.

 This is a low-memory helper for embedded readers.  It avoids allocating the
 whole RAWML buffer used by mobi_get_rawml()/mobi_parse_rawml_opt().
 */
MOBI_RET mobi_get_rawml_record(const MOBIData *m, size_t record_index, char *text, size_t *len) {
    if (m == NULL) {
        debug_print("%s", "Mobi structure not initialized\n");
        return MOBI_INIT_FAILED;
    }
    ...
    if (m->rh == NULL || m->rh->text_record_count == 0 || record_index >= m->rh->text_record_count) {
        debug_print("%s", "Text record index out of range\n");
        return MOBI_DATA_CORRUPT;
    }

    const size_t offset = mobi_get_kf8offset(m);
    const size_t text_rec_index = 1 + offset + record_index;
    const MOBIPdbRecord *curr = mobi_get_record_by_seqnumber(m, text_rec_index);
    if (curr == NULL) {
        debug_print("%s", "Text record not found\n");
        return MOBI_DATA_CORRUPT;
    }
    ...
    out_size = out_capacity - 1;
    text[0] = '\0';

    switch (compression_type) {
        case MOBI_COMPRESSION_NONE:
            if (record_size > out_size) {
                return MOBI_PARAM_ERR;
            }
            memcpy(text, curr->data, record_size);
            out_size = record_size;
            if (mobi_exists_mobiheader(m) && mobi_get_fileversion(m) <= 3) {
                mobi_remove_zeros((unsigned char *) text, &out_size);
            }
            break;
        case MOBI_COMPRESSION_PALMDOC:
            ret = mobi_decompress_lz77((unsigned char *) text, curr->data, &out_size, record_size);
            if (ret != MOBI_SUCCESS) {
                return ret;
            }
            break;
        case MOBI_COMPRESSION_HUFFCDIC: {
            MOBIHuffCdic *huffcdic = mobi_init_huffcdic();
            if (huffcdic == NULL) {
                debug_print("%s\n", "Memory allocation failed");
                return MOBI_MALLOC_FAILED;
            }
            ...
            if (ret == MOBI_SUCCESS) {
                ret = mobi_decompress_huffman((unsigned char *) text, curr->data, &out_size, record_size, huffcdic);
            }
            mobi_free_huffcdic(huffcdic);
            if (ret != MOBI_SUCCESS) {
                return ret;
...
MOBIFiletype mobi_determine_resource_type(const MOBIPdbRecord *record) {
    ...
    if (record == NULL || record->data == NULL || record->size < 4) {
        return T_UNKNOWN;
    }
...
size_t mobi_get_kf8boundary_seqnumber(const MOBIData *m) {
    ...
    if (exth_tag != NULL) {
        ...
        if (record && record->data && record->size >= sizeof(BOUNDARY_MAGIC) - 1) {
