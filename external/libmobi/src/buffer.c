...
static bool mobi_buffer_has_data(MOBIBuffer *buf) {
    if (!buf || !buf->data) {
        debug_print("%s", "Buffer not initialized\n");
        if (buf) {
...
uint8_t mobi_buffer_get8(MOBIBuffer *buf) {
    ...
    if (buf->offset > buf->maxlen || buf->maxlen - buf->offset < 1) {
...
uint16_t mobi_buffer_get16(MOBIBuffer *buf) {
    ...
    if (buf->offset > buf->maxlen || buf->maxlen - buf->offset < 2) {
        debug_print("%s", "End of buffer\n");
        buf->error = MOBI_BUFFER_END;
        return 0;
    }
...
uint32_t mobi_buffer_get32(MOBIBuffer *buf) {
    ...
    if (buf->offset > buf->maxlen || buf->maxlen - buf->offset < 4) {
        debug_print("%s", "End of buffer\n");
        buf->error = MOBI_BUFFER_END;
        return 0;
    }
...
static uint32_t mobi_buffer_get_varlen_internal(MOBIBuffer *buf, size_t *len, const int direction) {
    ...
    size_t max_count;
    if (!len || !mobi_buffer_has_data(buf)) {
        return 0;
    }
    max_count = direction == 1 ? buf->maxlen - buf->offset : buf->offset;
...
void mobi_buffer_getstring(char *str, MOBIBuffer *buf, const size_t len) {
    ...
    if (buf->offset > buf->maxlen || len > buf->maxlen - buf->offset) {
        debug_print("%s", "End of buffer\n");
        buf->error = MOBI_BUFFER_END;
        str[0] = '\0';
        return;
...
void mobi_buffer_copy(MOBIBuffer *dest, MOBIBuffer *source, const size_t len) {
    if (!mobi_buffer_has_data(source) || !mobi_buffer_has_data(dest)) {
        return;
    }
...
bool mobi_buffer_match_magic(MOBIBuffer *buf, const char *magic) {
    size_t magic_length;
    if (!magic || !mobi_buffer_has_data(buf)) {
        return false;
    }
