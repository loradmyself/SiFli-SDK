...
static char *szMessage =
	...
	!defined(_MSC_VER) && !defined(WIN32) && !defined(_WIN32)
#define ANTIWORD_REAL_MODE_DOS_LIMIT 1
static char *szDosMessage =
	"DOS can't allocate this kind of memory, unable to continue";
#endif /* real-mode DOS allocation limit */
...
#if defined(ANTIWORD_REAL_MODE_DOS_LIMIT)
	if ((ULONG)tNmemb * (ULONG)tSize > 0xffffUL) {
		DBG_DEC((ULONG)tNmemb * (ULONG)tSize);
		werr(1, szDosMessage);
	}
#endif /* ANTIWORD_REAL_MODE_DOS_LIMIT */
