 ...
 */
static char *
szLpstr(ULONG ulOffset, const UCHAR *aucBuffer, size_t tBufferLen)
{
	const UCHAR	*pucStart, *pucEnd;
	char	*szResult;
	size_t	tSize, tLen;

	if (aucBuffer == NULL || ulOffset > tBufferLen ||
	    tBufferLen - (size_t)ulOffset < 8) {
		return NULL;
	}
	...
	pucStart = aucBuffer + ulOffset + 8;
	pucEnd = aucBuffer + tBufferLen;
	if (tSize > (size_t)(pucEnd - pucStart)) {
		DBG_DEC(tSize);
		tSize = (size_t)(pucEnd - pucStart);
	}
	pucEnd = pucStart + tSize;
	/* Remove white space from the start of the string.  The property string is
	 * length-prefixed and is not guaranteed to be safely NUL-terminated inside
	 * aucBuffer, so never call strlen/xstrdup on the raw buffer. */
	while (pucStart < pucEnd && *pucStart != '\0' && isspace((unsigned char)*pucStart)) {
		pucStart++;
	}
	while (pucEnd > pucStart && pucEnd[-1] == '\0') {
		pucEnd--;
	}
	while (pucEnd > pucStart && isspace((unsigned char)pucEnd[-1])) {
		pucEnd--;
	}
	if (pucStart >= pucEnd || *pucStart == '\0') {
		return NULL;
	}
	tLen = 0;
	while (pucStart + tLen < pucEnd && pucStart[tLen] != '\0') {
		tLen++;
	}
	if (tLen == 0) {
		return NULL;
	}
	szResult = xmalloc(tLen + 1);
	memcpy(szResult, pucStart, tLen);
	szResult[tLen] = '\0';
	NO_DBG_MSG(szResult);
	return szResult;
} /* end of szLpstr */
 ...
 */
static void
vAnalyseSummaryInfo(const UCHAR *aucBuffer, size_t tBufferLen)
{
	ULONG	ulOffset;
	size_t	tIndex, tCount, tPropID, tPropType, tPropPos;

	if (aucBuffer == NULL || tBufferLen < 8) {
		return;
	}
	...
	for (tIndex = 0; tIndex < tCount; tIndex++) {
		tPropPos = 8 + tIndex * 8;
		if (tPropPos > tBufferLen || tBufferLen - tPropPos < 8) {
			DBG_DEC(tIndex);
			break;
		}
		tPropID = (size_t)ulGetLong(tPropPos, aucBuffer);
		ulOffset = ulGetLong(tPropPos + 4, aucBuffer);
		NO_DBG_DEC(tPropID);
		NO_DBG_HEX(ulOffset);
		if (ulOffset > tBufferLen || tBufferLen - (size_t)ulOffset < 4) {
			DBG_HEX(ulOffset);
			continue;
		}
		...
		case PID_TITLE:
			if (tPropType == VT_LPSTR && szTitle == NULL) {
				szTitle = szLpstr(ulOffset, aucBuffer, tBufferLen);
			}
			break;
		case PID_SUBJECT:
			if (tPropType == VT_LPSTR && szSubject == NULL) {
				szSubject = szLpstr(ulOffset, aucBuffer, tBufferLen);
			}
			break;
		case PID_AUTHOR:
			if (tPropType == VT_LPSTR && szAuthor == NULL) {
				szAuthor = szLpstr(ulOffset, aucBuffer, tBufferLen);
			}
			break;
		case PID_CREATE_DTM:
		...
		case PID_APPNAME:
			if (tPropType == VT_LPSTR && szAppName == NULL) {
				szAppName = szLpstr(ulOffset, aucBuffer, tBufferLen);
			}
			break;
		default:
			break;
 ...
 */
static void
vAnalyseDocumentSummaryInfo(const UCHAR *aucBuffer, size_t tBufferLen)
{
	ULONG	ulOffset;
	size_t	tIndex, tCount, tPropID, tPropType, tPropPos;
	...
	for (tIndex = 0; tIndex < tCount; tIndex++) {
		...
		case PIDD_MANAGER:
			if (tPropType == VT_LPSTR && szManager == NULL) {
				szManager = szLpstr(ulOffset, aucBuffer, tBufferLen);
			}
			break;
		case PIDD_COMPANY:
			if (tPropType == VT_LPSTR && szCompany == NULL) {
				szCompany = szLpstr(ulOffset, aucBuffer, tBufferLen);
			}
			break;
		default:
			break;
	...
	const ULONG *aulSBD, size_t tSBDLen)
{
	...
	pucBuffer = pucAnalyseSummaryInfoHeader(pFile,
		...
		aulBBD, tBBDLen, aulSBD, tSBDLen, &tBufferLen);
	if (pucBuffer != NULL) {
		vAnalyseSummaryInfo(pucBuffer, tBufferLen);
	...
	pucBuffer = pucAnalyseSummaryInfoHeader(pFile,
		...
		aulBBD, tBBDLen, aulSBD, tSBDLen, &tBufferLen);
	if (pucBuffer != NULL) {
		vAnalyseDocumentSummaryInfo(pucBuffer, tBufferLen);
		pucBuffer = xfree(pucBuffer);
	}
