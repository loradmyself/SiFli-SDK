...
pOpenCharacterMappingFile(const char *szLeafname)
{
	...
	if (STRCEQ(szLeafname, MAPPING_FILE_UTF_8)) {
		/*
		 * UTF-8 output is generated algorithmically by Antiword.  The original
		 * package ships UTF-8.txt as a dummy mapping file, but embedded/simulator
		 * builds do not always deploy C:\\antiword or $ANTIWORDHOME resources.
		 * Suppress the misleading missing-file warning and let iReadOptions()
		 * accept UTF-8 below.
		 */
		return NULL;
	}
...
/* Command line */
	...
	if (pCharacterMappingFile != NULL) {
		bSuccess = bReadCharacterMappingTable(pCharacterMappingFile);
		vCloseCharacterMappingFile(pCharacterMappingFile);
	} else if (STRCEQ(szLeafname, MAPPING_FILE_UTF_8)) {
		/*
		 * UTF-8 is handled algorithmically by Antiword and does not need
		 * a real mapping table. The distributed UTF-8.txt is only a dummy
		 * file, so allow embedded reader builds to run without deploying it.
		 */
		bSuccess = TRUE;
	} else {
		bSuccess = FALSE;
