                            ...
                            FX_LPBYTE& dest_buf, FX_DWORD& dest_size, FX_DWORD& offset)
{
    ...
    while (1) {
        ...
        if (!useOldImpl) {
            ...
            if (avail_buf_size == 0) {
                ...
                FX_LPBYTE new_guess_buf = FX_Realloc(FX_BYTE, guess_buf, guess_size + 1);
                if (!new_guess_buf) {
                    FPDFAPI_FlateEnd(context);
                    FX_Free(guess_buf);
                    dest_buf = NULL;
    ...
    if (!useOldImpl) {
        ...
        if (guess_size / 2 > dest_size) {
            FX_LPBYTE new_guess_buf = FX_Realloc(FX_BYTE, guess_buf, dest_size + 1);
            if (!new_guess_buf) {
                FPDFAPI_FlateEnd(context);
                FX_Free(guess_buf);
                dest_buf = NULL;
