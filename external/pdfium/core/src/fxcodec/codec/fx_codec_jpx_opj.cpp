...
static void sycc_to_rgb(int offset, int upb, int y, int cb, int cr,
                        OPJ_INT32 *out_r, OPJ_INT32 *out_g, OPJ_INT32 *out_b)
{
    int r, g, b;
    cb -= offset;
    cr -= offset;
...
{
    OPJ_INT32 *d0, *d1, *d2, *r, *g, *b;
    const OPJ_INT32 *y, *cb, *cr;
    ...
    cb = img->comps[1].data;
    cr = img->comps[2].data;
    d0 = r = FX_Alloc(OPJ_INT32, (size_t)max);
    d1 = g = FX_Alloc(OPJ_INT32, (size_t)max);
    d2 = b = FX_Alloc(OPJ_INT32, (size_t)max);
    for(i = 0; i < max; ++i) {
...
{
    OPJ_INT32 *d0, *d1, *d2, *r, *g, *b, *nr, *ng, *nb;
    const OPJ_INT32 *y, *cb, *cr, *ny;
    int maxw, maxh, max, offset, upb;
    int i, j;
    i = (int)img->comps[0].prec;
    offset = 1 << (i - 1);
...
void color_apply_icc_profile(opj_image_t *image)
{
    ...
    if(image->numcomps > 2) {
        ...
        in = inbuf = FX_Alloc(unsigned char, nr_samples);
        out = outbuf = FX_Alloc(unsigned char, nr_samples);
        opj_image_comp_t* new_comps = (opj_image_comp_t*)
                       FXMEM_DefaultReallocDebug(image->comps, (image->numcomps + 2) * sizeof(opj_image_comp_t), 0, __FILE__, __LINE__);
        if (!new_comps) {
            FX_Free(inbuf);
            FX_Free(outbuf);
            return;
        ...
        image->comps[1].data = FX_Alloc(OPJ_INT32, (size_t)max);
        FXSYS_memset8(image->comps[1].data, 0, sizeof(OPJ_INT32) * (size_t)max);
        image->comps[2].data = FX_Alloc(OPJ_INT32, (size_t)max);
        FXSYS_memset8(image->comps[2].data, 0, sizeof(OPJ_INT32) * (size_t)max);
        image->numcomps += 2;
        r = image->comps[0].data;
        for(i = 0; i < max; ++i) {
            *in++ = (unsigned char) * r++;
...
void color_apply_conversion(opj_image_t *image)
{
    ...
    enumcs = row[0];
    if(enumcs == 14) {
        OPJ_INT32 *L, *a, *b, *red, *green, *blue, *src0, *src1, *src2;
        ...
        red = FX_Alloc(OPJ_INT32, max);
        image->comps[0].data = red;
        green = FX_Alloc(OPJ_INT32, max);
        image->comps[1].data = green;
        blue = FX_Alloc(OPJ_INT32, max);
