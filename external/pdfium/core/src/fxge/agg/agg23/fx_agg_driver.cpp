...
{
    void* pDriverBuf = FXMEM_DefaultCallocDebug(1, sizeof(CFX_AggDeviceDriver), 0, __FILE__, __LINE__);
    if (!pDriverBuf) {
        return NULL;
    }
    return new (pDriverBuf) CFX_AggDeviceDriver(pBitmap, 0, bRgbByteOrder, pOriDevice, bGroupKnockout);
}
...
FX_BOOL CFX_FxgeDevice::Attach(CFX_DIBitmap* pBitmap, int dither_bits, FX_BOOL bRgbByteOrder, CFX_DIBitmap* pOriDevice, FX_BOOL bGroupKnockout)
{
    ...
    IFX_RenderDeviceDriver* pDriver = new (pDriverBuf) CFX_AggDeviceDriver(pBitmap, dither_bits, bRgbByteOrder, pOriDevice, bGroupKnockout);
    if (!pDriver) {
        return FALSE;
    }
...
FX_BOOL CFX_FxgeDevice::Create(int width, int height, FXDIB_Format format, int dither_bits, CFX_DIBitmap* pOriDevice)
{
    ...
    IFX_RenderDeviceDriver* pDriver = new (pDriverBuf) CFX_AggDeviceDriver(pBitmap, dither_bits, FALSE, pOriDevice, FALSE);
    if (!pDriver) {
        delete pBitmap;
        return FALSE;
    }
