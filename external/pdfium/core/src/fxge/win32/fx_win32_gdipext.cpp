...
// Windows SDK GDI+ headers rely on COM types (e.g. IStream, PROPID). Depending
// on preprocessor settings (notably WIN32_LEAN_AND_MEAN) and include order,
// these may not be visible from <windows.h> alone in some build environments.
// Include them explicitly before <gdiplus.h> to avoid syntax cascades in
// GdiplusImaging.h / GdiplusHeaders.h.
#include <objidl.h>   // IStream, ISequentialStream
