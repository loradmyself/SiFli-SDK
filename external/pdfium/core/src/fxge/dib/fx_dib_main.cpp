                                 ...
                                 int alpha_flag, void* pIccTransform, int blend_type)
{
    ...
    PDF_DIB_LOG("ImageRenderer::Start this=%p src=%p device=%p clip=[%d,%d,%d,%d] image_rect=[%d,%d,%d,%d] matrix=[%d %d %d %d %d %d] flags=0x%x alpha=%d blend=%d rgb_order=%d",
                this, pSource, pDevice,
                m_ClipBox.left, m_ClipBox.top, m_ClipBox.right, m_ClipBox.bottom,
                image_rect.left, image_rect.top, image_rect.right, image_rect.bottom,
                (int)m_Matrix.a, (int)m_Matrix.b, (int)m_Matrix.c,
                (int)m_Matrix.d, (int)m_Matrix.e, (int)m_Matrix.f,
                dib_flags, bitmap_alpha, blend_type, bRgbByteOrder);
    if (m_ClipBox.IsEmpty()) {
        PDF_DIB_LOG("ImageRenderer::Start empty_clip this=%p", this);
        return FALSE;
    }
    m_pDevice = pDevice;
    m_pClipRgn = pClipRgn;
    ...
    if ((FXSYS_fabs(m_Matrix.b) >= 0.5f || m_Matrix.a == 0) ||
        ...
        if (!m_pTransformer) {
            PDF_DIB_LOG("ImageRenderer::Start transformer_alloc_failed this=%p", this);
            return FALSE;
        }
        m_pTransformer->Start(pSource, &m_Matrix, dib_flags, &m_ClipBox);
        PDF_DIB_LOG("ImageRenderer::Start transformer_path this=%p status=%d", this, m_Status);
        return TRUE;
    }
    int dest_width = image_rect.Width();
    if (m_Matrix.a < 0) {
    ...
    PDF_DIB_LOG("ImageRenderer::Start stretcher_path this=%p status=%d ret=%d dest=%dx%d clip=[%d,%d,%d,%d]",
                this, m_Status, ret, dest_width, dest_height,
                bitmap_clip.left, bitmap_clip.top, bitmap_clip.right, bitmap_clip.bottom);
    return ret;
}
FX_BOOL CFX_ImageRenderer::Continue(IFX_Pause* pPause)
{
    if (m_Status == 1) {
        FX_BOOL ret = m_Stretcher.Continue(pPause);
        PDF_DIB_LOG("ImageRenderer::Continue stretcher this=%p ret=%d", this, ret);
        return ret;
    } else if (m_Status == 2) {
        FX_BOOL ret = m_pTransformer->Continue(pPause);
        PDF_DIB_LOG("ImageRenderer::Continue transformer this=%p ret=%d result_left=%d result_top=%d",
                    this, ret, m_pTransformer->m_ResultLeft, m_pTransformer->m_ResultTop);
        if (ret) {
            return TRUE;
        }
        CFX_DIBitmap* pBitmap = m_pTransformer->m_Storer.Detach();
        if (pBitmap == NULL) {
            PDF_DIB_LOG("ImageRenderer::Continue detach_null this=%p", this);
            return FALSE;
        }
        if (pBitmap->GetBuffer() == NULL) {
            PDF_DIB_LOG("ImageRenderer::Continue bitmap_no_buffer this=%p bmp=%p w=%d h=%d bpp=%d",
                        this, pBitmap, pBitmap->GetWidth(), pBitmap->GetHeight(), pBitmap->GetBPP());
            delete pBitmap;
            return FALSE;
        }
        PDF_DIB_LOG("ImageRenderer::Continue composite this=%p bmp=%p w=%d h=%d bpp=%d alpha_mask=%d left=%d top=%d first=%u,%u,%u,%u",
                    this, pBitmap, pBitmap->GetWidth(), pBitmap->GetHeight(), pBitmap->GetBPP(),
                    pBitmap->IsAlphaMask(), m_pTransformer->m_ResultLeft, m_pTransformer->m_ResultTop,
                    pBitmap->GetBuffer()[0], pBitmap->GetBuffer()[1], pBitmap->GetBuffer()[2], pBitmap->GetBuffer()[3]);
    ...
    PDF_DIB_LOG("ImageRenderer::Continue invalid_status this=%p status=%d", this, m_Status);
    return FALSE;
}
