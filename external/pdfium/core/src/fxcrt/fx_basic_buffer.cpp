...
{
    FX_STRSIZE old_size = m_DataSize;
    ExpandBuf(size);
    if (!m_pBuffer || m_AllocSize < old_size + size) {
        return;
    }
    if (pBuf) {
        FXSYS_memcpy32(m_pBuffer + old_size, pBuf, size);
    }
    m_DataSize = old_size + size;
}
void CFX_BinaryBuf::InsertBlock(FX_STRSIZE pos, const void* pBuf, FX_STRSIZE size)
{
    ...
    FXSYS_memmove32(m_pBuffer + pos + size, m_pBuffer + pos, old_size - pos);
    if (pBuf) {
...
void CFX_BinaryBuf::AppendFill(FX_BYTE byte, FX_STRSIZE count)
{
    FX_STRSIZE old_size = m_DataSize;
    ExpandBuf(count);
    if (!m_pBuffer || m_AllocSize < old_size + count) {
        return;
    }
    FXSYS_memset8(m_pBuffer + old_size, byte, count);
    m_DataSize = old_size + count;
}
...
void CFX_WideTextBuf::AppendChar(FX_WCHAR ch)
{
    ...
    if (!m_pBuffer || m_AllocSize < m_DataSize + (FX_STRSIZE)sizeof(FX_WCHAR)) {
        return;
    }
...
CFX_WideTextBuf& CFX_WideTextBuf::operator << (int i)
{
    ...
    if (!m_pBuffer || m_AllocSize < m_DataSize + (FX_STRSIZE)(len * sizeof(FX_WCHAR))) {
        return *this;
    }
