...
static FX_BOOL _CompactStringStore(_CompactString* pCompact, FX_LPCBYTE pStr, int len)
{
...
FX_BOOL CFX_CMapByteStringToPtr::SetAt(FX_BSTR key, void* value)
{
    ASSERT(value != NULL);
    ...
    for (index = 0; index < size; index ++) {
        ...
        if (!_CompactStringStore(pKey, (FX_LPCBYTE)key, key_len)) {
            return FALSE;
        }
        *(void**)(pKey + 1) = value;
        return TRUE;
...
FX_BOOL CFX_CMapByteStringToPtr::AddValue(FX_BSTR key, void* value)
{
    ASSERT(value != NULL);
    _CompactString* pKey = (_CompactString*)m_Buffer.Add();
    if (!pKey) {
    ...
    if (!_CompactStringStore(pKey, (FX_LPCBYTE)key, key.GetLength())) {
        return FALSE;
    }
    *(void**)(pKey + 1) = value;
    return TRUE;
