...
void CFX_ByteString::AssignCopy(FX_STRSIZE nSrcLen, FX_LPCSTR lpszSrcData)
{
    ...
    if (!m_pData || m_pData->m_nRefs > 1 || m_pData->m_nAllocLength < nSrcLen) {
        return;
    }
...
void CFX_ByteString::CopyBeforeWrite()
{
    ...
    CFX_StringData* pNewData = FX_AllocString(nDataLength);
    if (pNewData != NULL) {
        FXSYS_memcpy32(pNewData->m_String, pData->m_String, nDataLength + 1);
        pData->m_nRefs --;
        m_pData = pNewData;
    }
}
void CFX_ByteString::AllocBeforeWrite(FX_STRSIZE nLen)
{
    ...
    CFX_StringData* pNewData = FX_AllocString(nLen);
    if (!pNewData) {
        return;
    }
    Empty();
...
FX_LPSTR CFX_ByteString::GetBuffer(FX_STRSIZE nMinBufLength)
{
    ...
    CFX_StringData* pNewData = FX_AllocString(nMinBufLength);
    if (!pNewData) {
        return NULL;
    }
    FXSYS_memcpy32(pNewData->m_String, pOldData->m_String, (nOldLen + 1));
    pNewData->m_nDataLength = nOldLen;
    pOldData->m_nRefs --;
...
void CFX_ByteString::ConcatInPlace(FX_STRSIZE nSrcLen, FX_LPCSTR lpszSrcData)
{
    ...
    if (m_pData->m_nRefs > 1 || m_pData->m_nDataLength + nSrcLen > m_pData->m_nAllocLength) {
        CFX_StringData* pOldData = m_pData;
        int nNewLen = m_pData->m_nDataLength + nSrcLen;
        CFX_StringData* pNewData = FX_AllocString(nNewLen);
        if (!pNewData) {
            return;
        }
        FXSYS_memcpy32(pNewData->m_String, pOldData->m_String, pOldData->m_nDataLength);
        FXSYS_memcpy32(pNewData->m_String + pOldData->m_nDataLength, lpszSrcData, nSrcLen);
        m_pData = pNewData;
...
FX_STRSIZE CFX_ByteString::Insert(FX_STRSIZE nIndex, FX_CHAR ch)
{
    ...
    if (m_pData == NULL || m_pData->m_nAllocLength < nNewLength) {
        CFX_StringData* pOldData = m_pData;
        FX_LPCSTR pstr = pOldData ? pOldData->m_String : NULL;
