        ...
        const CFX_AffineMatrix* pTextMatrix, FX_BOOL bFill, FX_BOOL bStroke)
{
    if (!bStroke) {
        ...
        CFX_PathData* pPathData = path.m_Path.New();
        if (!pPathData || !pPathData->AppendRect(textobj->m_Left, textobj->m_Bottom, textobj->m_Right, textobj->m_Top)) {
            return;
        }
    ...
    for (FX_DWORD i = 0; i < CharPosList.m_nChars; i ++) {
        ...
        CFX_PathData* pPathData = path.m_Path.New();
        if (!pPathData || !pPathData->Append(pPath, &matrix)) {
            continue;
        }
