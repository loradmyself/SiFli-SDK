...
FX_BOOL CPDF_ImageRenderer::StartLoadDIBSource()
{
    ...
    PDF_RENDER_IMG_LOG("startload obj=%u dest_w=%d dest_h=%d clip=[l=%d t=%d r=%d b=%d]",
                       m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0,
                       dest_width, dest_height,
                       m_pRenderStatus->m_pDevice->GetClipBox().left,
                       m_pRenderStatus->m_pDevice->GetClipBox().top,
                       m_pRenderStatus->m_pDevice->GetClipBox().right,
                       m_pRenderStatus->m_pDevice->GetClipBox().bottom);
...
{
    PDF_RENDER_IMG_LOG("startrender obj=%u loader_bitmap=%p loader_mask=%p cached=%d matte=0x%x",
                       m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0,
                       m_Loader.m_pBitmap, m_Loader.m_pMask, m_Loader.m_bCached, m_Loader.m_MatteColor);
    if (m_Loader.m_pBitmap == NULL) {
        PDF_RENDER_IMG_LOG("startrender abort obj=%u reason=no_bitmap",
                           m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0);
        return FALSE;
...
#endif
    PDF_RENDER_IMG_LOG("startrender dib obj=%u dib=[w=%d h=%d bpp=%d alpha_mask=%d opaque=%d] bitmap_alpha=%d fill=0x%x flags=0x%x blend=%d pattern=%d mask=%p",
                       m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0,
                       m_pDIBSource ? m_pDIBSource->GetWidth() : 0,
                       m_pDIBSource ? m_pDIBSource->GetHeight() : 0,
                       m_pDIBSource ? m_pDIBSource->GetBPP() : 0,
                       m_pDIBSource ? m_pDIBSource->IsAlphaMask() : 0,
                       m_pDIBSource ? m_pDIBSource->IsOpaqueImage() : 0,
                       m_BitmapAlpha, m_FillArgb, m_Flags, m_BlendType,
                       m_bPatternColor, m_Loader.m_pMask);
    if (m_Loader.m_pMask) {
        PDF_RENDER_IMG_LOG("startrender choose masked obj=%u",
                           m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0);
    ...
    if (m_bPatternColor) {
        PDF_RENDER_IMG_LOG("startrender choose pattern obj=%u",
                           m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0);
...
#endif
    PDF_RENDER_IMG_LOG("startrender choose dib obj=%u",
                       m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0);
...
FX_BOOL CPDF_ImageRenderer::Start(CPDF_RenderStatus* pStatus, const CPDF_PageObject* pObj, const CFX_AffineMatrix* pObj2Device, FX_BOOL bStdCS, int blendType)
{
    ...
    m_ImageMatrix.Concat(*pObj2Device);
    {
        CFX_FloatRect image_rect_f = m_ImageMatrix.GetUnitRect();
        FX_RECT image_rect = image_rect_f.GetOutterRect();
        PDF_RENDER_IMG_LOG("obj=%u matrix=[%d %d %d %d %d %d] rect=[l=%d t=%d r=%d b=%d w=%d h=%d]",
                           m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0,
                           (int)m_ImageMatrix.a, (int)m_ImageMatrix.b,
                           (int)m_ImageMatrix.c, (int)m_ImageMatrix.d,
                           (int)m_ImageMatrix.e, (int)m_ImageMatrix.f,
                           image_rect.left, image_rect.top, image_rect.right,
                           image_rect.bottom, image_rect.Width(), image_rect.Height());
    }
...
FX_BOOL CPDF_ImageRenderer::StartDIBSource()
{
    ...
    if (m_pRenderStatus->m_pDevice->StartDIBits(m_pDIBSource, m_BitmapAlpha, m_FillArgb,
        ...
        PDF_RENDER_IMG_LOG("startdibits accepted obj=%u handle=%p alpha=%d flags=0x%x blend=%d dib=[w=%d h=%d bpp=%d]",
                           m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0,
                           m_DeviceHandle, m_BitmapAlpha, m_Flags, m_BlendType,
                           m_pDIBSource ? m_pDIBSource->GetWidth() : 0,
                           m_pDIBSource ? m_pDIBSource->GetHeight() : 0,
                           m_pDIBSource ? m_pDIBSource->GetBPP() : 0);
    ...
    int dest_width = image_rect.Width();
    int dest_height = image_rect.Height();
    PDF_RENDER_IMG_LOG("startdibits fallback obj=%u rect=[l=%d t=%d r=%d b=%d w=%d h=%d] clip=[l=%d t=%d r=%d b=%d] matrix=[%d %d %d %d %d %d]",
                       m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0,
    ...
    PDF_RENDER_IMG_LOG("stretch path obj=%u dest_left=%d dest_top=%d dest_w=%d dest_h=%d opaque=%d alpha_mask=%d alpha=%d",
                       m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0,
                       dest_left, dest_top, dest_width, dest_height,
                       m_pDIBSource ? m_pDIBSource->IsOpaqueImage() : 0,
                       m_pDIBSource ? m_pDIBSource->IsAlphaMask() : 0,
                       m_BitmapAlpha);
    ...
    PDF_RENDER_IMG_LOG("composite path obj=%u dest_rect=[l=%d t=%d r=%d b=%d w=%d h=%d] dest_clip=[l=%d t=%d r=%d b=%d]",
                       m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                           ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                           : 0,
                       dest_rect.left, dest_rect.top, dest_rect.right, dest_rect.bottom,
                       dest_rect.Width(), dest_rect.Height(),
                       dest_clip.left, dest_clip.top, dest_clip.right, dest_clip.bottom);
    CFX_DIBitmap* pStretched = m_pDIBSource->StretchTo(dest_width, dest_height, m_Flags, &dest_clip);
    if (pStretched) {
        PDF_RENDER_IMG_LOG("composite stretch ok obj=%u stretched=[w=%d h=%d bpp=%d] at=(%d,%d)",
                           m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0,
                           pStretched->GetWidth(), pStretched->GetHeight(), pStretched->GetBPP(),
        ...
        PDF_RENDER_IMG_LOG("composite stretch failed obj=%u", m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                                                          ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                                                          : 0);
    }
#endif
...
FX_BOOL CPDF_ImageRenderer::Continue(IFX_Pause* pPause)
{
    if (m_Status == 1) {
        ...
        PDF_RENDER_IMG_LOG("continue dibits obj=%u handle=%p",
                           m_pImageObject && m_pImageObject->m_pImage && m_pImageObject->m_pImage->GetStream()
                               ? m_pImageObject->m_pImage->GetStream()->GetObjNum()
                               : 0,
                           m_DeviceHandle);
