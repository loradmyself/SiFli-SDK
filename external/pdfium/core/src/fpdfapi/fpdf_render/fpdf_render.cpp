...
#if defined(SOLUTION)
#include "rtthread.h"
#define PDF_RENDER_OOM_LOG(fmt, ...) rt_kprintf("pdfium: " fmt "\n", ##__VA_ARGS__)
#else
#define PDF_RENDER_OOM_LOG(fmt, ...)
#endif
...
void CPDF_ProgressiveRenderer::Continue(IFX_Pause* pPause)
{
    ...
    for (; m_LayerIndex < nLayers; m_LayerIndex ++) {
        ...
        if (m_pRenderer == NULL) {
            m_ObjectPos = pItem->m_pObjectList->GetFirstObjectPosition();
            m_ObjectIndex = 0;
            void* pRendererBuf = FXMEM_DefaultCallocDebug(1, sizeof(CPDF_RenderStatus), 0, __FILE__, __LINE__);
            if (!pRendererBuf) {
                PDF_RENDER_OOM_LOG("%s alloc CPDF_RenderStatus failed", __func__);
                m_Status = Failed;
                return;
            }
            m_pRenderer = new (pRendererBuf) CPDF_RenderStatus();
