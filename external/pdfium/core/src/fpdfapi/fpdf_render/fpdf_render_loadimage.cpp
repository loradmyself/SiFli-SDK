...
{
    PDF_IMAGE_LOG("LoadColorInfo obj=%u width=%d height=%d has_cs=%d image_mask=%d bpc=%d",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  m_pDict ? m_pDict->GetInteger(FX_BSTRC("Width")) : 0,
                  m_pDict ? m_pDict->GetInteger(FX_BSTRC("Height")) : 0,
                  m_pDict ? m_pDict->KeyExist(FX_BSTRC("ColorSpace")) : 0,
                  m_pDict ? m_pDict->GetInteger(FX_BSTRC("ImageMask")) : 0,
                  m_pDict ? m_pDict->GetInteger(FX_BSTRC("BitsPerComponent")) : 0);
        ...
        if (!m_bImageMask) {
            ...
            if (pFilter) {
                ...
                if (pFilter->GetType() == PDFOBJ_NAME) {
                    ...
                    PDF_IMAGE_LOG("LoadColorInfo obj=%u missing colorspace, filter=%s type=name",
                                  m_pStream ? m_pStream->GetObjNum() : 0,
                                  filter.IsEmpty() ? "<empty>" : (const char*)filter);
                    if (filter == FX_BSTRC("JPXDecode")) {
                        PDF_IMAGE_LOG("LoadColorInfo obj=%u allow JPX without explicit colorspace",
                                      m_pStream ? m_pStream->GetObjNum() : 0);
                        return TRUE;
                    }
                    ...
                    PDF_IMAGE_LOG("LoadColorInfo obj=%u missing colorspace, last_filter=%s type=array count=%d",
                                  m_pStream ? m_pStream->GetObjNum() : 0,
                                  filter.IsEmpty() ? "<empty>" : (const char*)filter,
                                  pArray->GetCount());
                    if (filter == FX_BSTRC("JPXDecode")) {
                        PDF_IMAGE_LOG("LoadColorInfo obj=%u allow JPX array without explicit colorspace",
                                      m_pStream ? m_pStream->GetObjNum() : 0);
                        return TRUE;
                    }
                }
            } else {
                PDF_IMAGE_LOG("LoadColorInfo obj=%u missing colorspace and missing filter",
                              m_pStream ? m_pStream->GetObjNum() : 0);
            }
        }
        m_bImageMask = TRUE;
    ...
    PDF_IMAGE_LOG("LoadColorInfo obj=%u colorspace family=%d comps=%d bpc=%d smask=%d mask=%d",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  m_Family, m_nComponents, m_bpc,
                  m_pDict->KeyExist(FX_BSTRC("SMask")),
                  m_pDict->KeyExist(FX_BSTRC("Mask")));
...
{
    const CFX_ByteString& decoder = m_pStreamAcc->GetImageDecoder();
    PDF_IMAGE_LOG("CreateDecoder obj=%u decoder=%s width=%d height=%d comps=%d bpc=%d data=%u",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  decoder.IsEmpty() ? "<none>" : (const char*)decoder,
                  m_Width, m_Height, m_nComponents, m_bpc,
                  m_pStreamAcc ? m_pStreamAcc->GetSize() : 0);
    if (decoder.IsEmpty()) {
        return 1;
    }
    ...
    if (m_pDecoder) {
        ...
        PDF_IMAGE_LOG("CreateDecoder ok obj=%u decoder=%s out_w=%d out_h=%d out_comps=%d out_bpc=%d req_pitch=%d got_pitch=%d",
                      m_pStream ? m_pStream->GetObjNum() : 0,
                      decoder.IsEmpty() ? "<none>" : (const char*)decoder,
                      m_pDecoder->GetWidth(), m_pDecoder->GetHeight(),
                      m_pDecoder->CountComps(), m_pDecoder->GetBPC(),
                      requested_pitch, provided_pitch);
        if (provided_pitch < requested_pitch) {
            PDF_IMAGE_LOG("CreateDecoder fail obj=%u decoder=%s provided_pitch_lt_requested",
                          m_pStream ? m_pStream->GetObjNum() : 0,
                          decoder.IsEmpty() ? "<none>" : (const char*)decoder);
            return 0;
        }
    ...
    PDF_IMAGE_LOG("CreateDecoder fail obj=%u decoder=%s no_decoder_created",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  decoder.IsEmpty() ? "<none>" : (const char*)decoder);
    return 0;
}
void CPDF_DIBSource::LoadJpxBitmap()
{
    ...
    if (pJpxModule == NULL) {
        PDF_IMAGE_LOG("LoadJpxBitmap obj=%u no_jpx_module",
                      m_pStream ? m_pStream->GetObjNum() : 0);
        return;
    }
    FX_LPVOID ctx = pJpxModule->CreateDecoder(m_pStreamAcc->GetData(), m_pStreamAcc->GetSize(), m_pColorSpace != NULL);
    if (ctx == NULL) {
        PDF_IMAGE_LOG("LoadJpxBitmap obj=%u create_decoder_failed size=%u has_colorspace=%d",
                      m_pStream ? m_pStream->GetObjNum() : 0,
                      m_pStreamAcc ? m_pStreamAcc->GetSize() : 0,
                      m_pColorSpace != NULL);
        return;
    ...
    PDF_IMAGE_LOG("LoadJpxBitmap obj=%u info dict_w=%d dict_h=%d jpx_w=%u jpx_h=%u code_comps=%u img_comps=%u has_colorspace=%d",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  m_Width, m_Height, width, height, codestream_nComps, image_nComps,
                  m_pColorSpace != NULL);
    if ((int)width < m_Width || (int)height < m_Height) {
        PDF_IMAGE_LOG("LoadJpxBitmap obj=%u reject_smaller_codestream",
                      m_pStream ? m_pStream->GetObjNum() : 0);
    ...
    if (m_pColorSpace) {
        if (codestream_nComps != (FX_DWORD)m_pColorSpace->CountComponents()) {
            PDF_IMAGE_LOG("LoadJpxBitmap obj=%u colorspace_comp_mismatch cs_comps=%d code_comps=%u",
                          m_pStream ? m_pStream->GetObjNum() : 0,
                          m_pColorSpace->CountComponents(), codestream_nComps);
            return;
        }
    ...
    m_pCachedBitmap = FX_NEW CFX_DIBitmap;
    PDF_IMAGE_LOG("LoadJpxBitmap obj=%u decoded_format=%d output_comps=%d translate=%d swap_rgb=%d width=%u height=%u",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  format, output_nComps, bTranslateColor, bSwapRGB, width, height);
    if (!m_pCachedBitmap->Create(width, height, format)) {
        PDF_IMAGE_LOG("LoadJpxBitmap obj=%u bitmap_create_failed width=%u height=%u format=%d",
                      m_pStream ? m_pStream->GetObjNum() : 0,
                      width, height, format);
        delete m_pCachedBitmap;
        m_pCachedBitmap = NULL;
    ...
    if (!pJpxModule->Decode(ctx, m_pCachedBitmap->GetBuffer(), m_pCachedBitmap->GetPitch(), bTranslateColor, output_offsets)) {
        PDF_IMAGE_LOG("LoadJpxBitmap obj=%u decode_failed pitch=%d",
                      m_pStream ? m_pStream->GetObjNum() : 0,
    ...
    PDF_IMAGE_LOG("LoadJpxBitmap obj=%u decode_ok pitch=%d bpc=%d comps=%d",
                  m_pStream ? m_pStream->GetObjNum() : 0,
                  m_pCachedBitmap->GetPitch(), m_bpc, m_nComponents);
...
FX_BOOL CPDF_ProgressiveImageLoaderHandle::Start(CPDF_ImageLoader* pImageLoader, const CPDF_ImageObject* pImage, CPDF_PageRenderCache* pCache, FX_BOOL bStdCS, FX_DWORD GroupFamily, FX_BOOL bLoadMask, CPDF_RenderStatus* pRenderStatus, FX_INT32 nDownsampleWidth, FX_INT32 nDownsampleHeight)
{
    ...
    FX_BOOL ret;
    if (pCache) {
        ret = pCache->StartGetCachedBitmap((CPDF_Stream*)pImage->m_pImage->GetStream(), bStdCS, GroupFamily, bLoadMask, pRenderStatus, nDownsampleWidth, nDownsampleHeight);
        PDF_IMAGE_LOG("LoaderHandle::Start obj=%u cache=1 ret=%d downsample=%dx%d",
                      pImage && pImage->m_pImage && pImage->m_pImage->GetStream()
                          ? pImage->m_pImage->GetStream()->GetObjNum()
                          : 0,
                      ret, m_nDownsampleWidth, m_nDownsampleHeight);
        if (ret == FALSE) {
            ...
            PDF_IMAGE_LOG("LoaderHandle::Start detach cache obj=%u bitmap=%p mask=%p matte=0x%x",
                          pImage && pImage->m_pImage && pImage->m_pImage->GetStream()
                              ? pImage->m_pImage->GetStream()->GetObjNum()
                              : 0,
                          m_pImageLoader->m_pBitmap, m_pImageLoader->m_pMask,
        ...
        PDF_IMAGE_LOG("LoaderHandle::Start obj=%u cache=0 ret=%d downsample=%dx%d",
                      pImage && pImage->m_pImage && pImage->m_pImage->GetStream()
                          ? pImage->m_pImage->GetStream()->GetObjNum()
                          : 0,
                      ret, m_nDownsampleWidth, m_nDownsampleHeight);
        if (ret == FALSE) {
            ...
            PDF_IMAGE_LOG("LoaderHandle::Start detach image obj=%u bitmap=%p mask=%p matte=0x%x",
                          pImage && pImage->m_pImage && pImage->m_pImage->GetStream()
                              ? pImage->m_pImage->GetStream()->GetObjNum()
                              : 0,
                          m_pImageLoader->m_pBitmap, m_pImageLoader->m_pMask,
...
FX_BOOL CPDF_ProgressiveImageLoaderHandle::Continue(IFX_Pause* pPause)
{
    ...
    if (m_pCache) {
        ...
        PDF_IMAGE_LOG("LoaderHandle::Continue obj=%u cache=1 ret=%d",
                      m_pImage && m_pImage->m_pImage && m_pImage->m_pImage->GetStream()
                          ? m_pImage->m_pImage->GetStream()->GetObjNum()
                          : 0,
                      ret);
        if (ret == FALSE) {
            m_pImageLoader->m_bCached = TRUE;
            ...
            PDF_IMAGE_LOG("LoaderHandle::Continue detach cache obj=%u bitmap=%p mask=%p matte=0x%x",
                          m_pImage && m_pImage->m_pImage && m_pImage->m_pImage->GetStream()
                              ? m_pImage->m_pImage->GetStream()->GetObjNum()
                              : 0,
                          m_pImageLoader->m_pBitmap, m_pImageLoader->m_pMask,
        ...
        PDF_IMAGE_LOG("LoaderHandle::Continue obj=%u cache=0 ret=%d",
                      m_pImage && m_pImage->m_pImage && m_pImage->m_pImage->GetStream()
                          ? m_pImage->m_pImage->GetStream()->GetObjNum()
                          : 0,
                      ret);
        if (ret == FALSE) {
            ...
            m_pImageLoader->m_pMask = m_pImage->m_pImage->DetachMask();
            m_pImageLoader->m_MatteColor = m_pImage->m_pImage->m_MatteColor;
            PDF_IMAGE_LOG("LoaderHandle::Continue detach image obj=%u bitmap=%p mask=%p matte=0x%x",
                          m_pImage && m_pImage->m_pImage && m_pImage->m_pImage->GetStream()
                              ? m_pImage->m_pImage->GetStream()->GetObjNum()
                              : 0,
                          m_pImageLoader->m_pBitmap, m_pImageLoader->m_pMask,
