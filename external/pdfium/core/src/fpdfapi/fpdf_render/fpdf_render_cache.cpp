...
int CPDF_ImageCache::ContinueGetCachedBitmap()
{
    ...
    if (m_pCurBitmap->GetPitch() * m_pCurBitmap->GetHeight() < FPDF_HUGE_IMAGE_SIZE) {
        CFX_DIBSource* pCachedBitmap = m_pCurBitmap->Clone();
        if (pCachedBitmap) {
            delete m_pCurBitmap;
            m_pCurBitmap = pCachedBitmap;
        }
    ...
    if (m_pCurMask) {
        CFX_DIBSource* pCachedMask = m_pCurMask->Clone();
        if (pCachedMask) {
            delete m_pCurMask;
            m_pCurMask = pCachedMask;
        }
...
#if _FXM_PLATFORM_  == _FXM_PLATFORM_APPLE_
        CFX_DIBSource* pCachedBitmap = m_pCurBitmap->CloneConvert(FXDIB_Rgb32);
#else
        CFX_DIBSource* pCachedBitmap = m_pCurBitmap->CloneConvert(FXDIB_Rgb);
#endif
        if (pCachedBitmap) {
            delete m_pCurBitmap;
            m_pCurBitmap = pCachedBitmap;
