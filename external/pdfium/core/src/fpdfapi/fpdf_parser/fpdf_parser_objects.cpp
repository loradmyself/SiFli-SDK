    ...
    switch (m_Type) {
        ...
        case PDFOBJ_ARRAY: {
                ...
                for (int i = 0; i < n; i ++) {
                    ...
                    CPDF_Object* pClone = value ? value->CloneInternal(bDirect, visited) : NULL;
                    if (!pClone) {
                        continue;
                    }
                    if (!pCopy->m_Objects.Add(pClone)) {
                        pClone->Release();
                        break;
                    }
                }
        ...
        case PDFOBJ_DICTIONARY: {
                ...
                while (pos) {
                    ...
                    if (!pCopy->m_Map.SetAt(key, pClone)) {
                        pClone->Release();
                        break;
                    }
                }
...
void CPDF_Array::SetAt(FX_DWORD i, CPDF_Object* pObj, CPDF_IndirectObjects* pObjs)
{
    ...
    if (pObj->GetObjNum()) {
        ASSERT(pObjs != NULL);
        CPDF_Object* pRef = CPDF_Reference::Create(pObjs, pObj->GetObjNum());
        if (!pRef) {
            return;
        }
        pObj = pRef;
...
void CPDF_Array::InsertAt(FX_DWORD index, CPDF_Object* pObj, CPDF_IndirectObjects* pObjs)
{
    ...
    if (!m_Objects.InsertAt(index, pObj)) {
        pObj->Release();
    }
}
...
{
    CPDF_Number* pNumber = FX_NEW CPDF_Number(i);
    if (!pNumber) {
        return;
    }
    SetAt(key, pNumber);
...
{
    CPDF_Name* pName = FX_NEW CPDF_Name(name);
    if (!pName) {
        return;
    }
    SetAt(key, pName);
...
{
    CPDF_String* pString = FX_NEW CPDF_String(str);
    if (!pString) {
        return;
    }
    SetAt(key, pString);
...
{
    CPDF_Reference* pRef = FX_NEW CPDF_Reference(pDoc, objnum);
    if (!pRef) {
        return;
    }
    SetAt(key, pRef);
...
{
    CPDF_Boolean* pBoolean = FX_NEW CPDF_Boolean(bValue);
    if (!pBoolean) {
        return;
    }
    SetAt(key, pBoolean);
...
void CPDF_Stream::InitStream(FX_LPBYTE pData, FX_DWORD size, CPDF_Dictionary* pDict)
{
    ...
    if (size && !m_pDataBuf) {
        m_dwSize = 0;
        if (m_pDict) {
            m_pDict->SetAtInteger(FX_BSTRC("Length"), 0);
        }
        return;
    }
    if (pData) {
