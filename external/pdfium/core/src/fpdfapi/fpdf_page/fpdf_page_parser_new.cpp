...
void CPDF_StreamContentParser::EndInlineImage()
{
    ...
    FX_BOOL bHasColorSpace = m_pLastCloneImageDict && m_pLastCloneImageDict->KeyExist(FX_BSTRC("ColorSpace"));
    ...
    SetGraphicStates(pImages, !bHasColorSpace, FALSE, FALSE);
...
void CPDF_ContentParser::Start(CPDF_Page* pPage, CPDF_ParseOptions* pOptions)
{
    ...
    if (!m_pParser || !m_pParser->Initialize()) {
        m_Status = Done;
        return;
    }
    ...
    CPDF_ColorStateData* pColorState = m_pParser->m_pCurStates->m_ColorState.GetModify();
    if (!pColorState) {
        m_Status = Done;
        return;
    }
