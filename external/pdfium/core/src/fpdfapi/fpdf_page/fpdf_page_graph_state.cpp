...
#if defined(SOLUTION)
#include <rtthread.h>
#define PDFIUM_GS_LOG(fmt, ...) \
    do { \
        if (FXMEM_HasOOM()) { \
            rt_kprintf("pdfium_gs: " fmt "\n", ##__VA_ARGS__); \
        } \
    } while (0)
#else
#define PDFIUM_GS_LOG(fmt, ...) do { } while (0)
#endif

void CPDF_GraphicStates::DefaultStates()
{
    PDFIUM_GS_LOG("DefaultStates enter this=0x%08x", (unsigned int)(FX_UINTPTR)this);
    CPDF_ColorStateData* pColorState = m_ColorState.New();
    if (pColorState) {
        pColorState->Default();
    } else {
        PDFIUM_GS_LOG("DefaultStates ColorState.New failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
    }
}
void CPDF_GraphicStates::CopyStates(const CPDF_GraphicStates& src)
{
    PDFIUM_GS_LOG("CopyStates this=0x%08x src=0x%08x", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)&src);
...
CPDF_ClipPathData::CPDF_ClipPathData(const CPDF_ClipPathData& src)
{
    ...
    if (m_PathCount) {
        ...
        if (!m_pTypeList) {
            FX_DELETE_VECTOR(m_pPathList, CPDF_Path, alloc_size);
            m_pPathList = NULL;
            m_PathCount = 0;
            return;
        }
...
{
    PDFIUM_GS_LOG("AppendPath this=0x%08x type=%d auto=%d", (unsigned int)(FX_UINTPTR)this, type, bAutoMerge);
    CPDF_ClipPathData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("AppendPath GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
    ...
    if (pData->m_PathCount % 8 == 0) {
        ...
        if (!pNewPath) {
            PDFIUM_GS_LOG("AppendPath new path list failed count=%d this=0x%08x", pData->m_PathCount + 8, (unsigned int)(FX_UINTPTR)this);
            return;
        }
        FX_BYTE* pNewType = FX_Alloc(FX_BYTE, pData->m_PathCount + 8);
        if (!pNewType) {
            PDFIUM_GS_LOG("AppendPath new type list failed count=%d this=0x%08x", pData->m_PathCount + 8, (unsigned int)(FX_UINTPTR)this);
            FX_DELETE_VECTOR(pNewPath, CPDF_Path, pData->m_PathCount + 8);
            return;
        }
...
{
    PDFIUM_GS_LOG("DeletePath this=0x%08x index=%d", (unsigned int)(FX_UINTPTR)this, index);
    CPDF_ClipPathData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("DeletePath GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
...
{
    PDFIUM_GS_LOG("AppendTexts this=0x%08x texts=0x%08x count=%d", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pTexts, count);
    CPDF_ClipPathData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("AppendTexts GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        for (int i = 0; i < count; i ++) {
            if (pTexts[i])
                pTexts[i]->Release();
        }
    ...
    if (!pNewList) {
        PDFIUM_GS_LOG("AppendTexts new list failed old=%d add=%d this=0x%08x", pData->m_TextCount, count, (unsigned int)(FX_UINTPTR)this);
        for (int i = 0; i < count; i ++) {
            if (pTexts[i])
                pTexts[i]->Release();
        }
...
{
    PDFIUM_GS_LOG("ClipPath Transform this=0x%08x", (unsigned int)(FX_UINTPTR)this);
    CPDF_ClipPathData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("ClipPath Transform GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
    int i;
...
{
    PDFIUM_GS_LOG("SetFillColor this=0x%08x cs=0x%08x values=0x%08x n=%d", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pCS, (unsigned int)(FX_UINTPTR)pValue, nValues);
    CPDF_ColorStateData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("SetFillColor GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
...
{
    PDFIUM_GS_LOG("SetStrokeColor this=0x%08x cs=0x%08x values=0x%08x n=%d", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pCS, (unsigned int)(FX_UINTPTR)pValue, nValues);
    CPDF_ColorStateData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("SetStrokeColor GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
...
void CPDF_ColorState::SetColor(CPDF_Color& color, FX_DWORD& rgb, CPDF_ColorSpace* pCS, FX_FLOAT* pValue, int nValues)
{
    ...
    if (color.m_pCS == NULL || color.m_pBuffer == NULL || pValue == NULL) {
        rgb = (FX_DWORD) - 1;
        return;
    }
...
{
    PDFIUM_GS_LOG("SetFillPattern this=0x%08x pattern=0x%08x values=0x%08x n=%d", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pPattern, (unsigned int)(FX_UINTPTR)pValue, nValues);
    CPDF_ColorStateData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("SetFillPattern GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
...
{
    PDFIUM_GS_LOG("SetStrokePattern this=0x%08x pattern=0x%08x values=0x%08x n=%d", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pPattern, (unsigned int)(FX_UINTPTR)pValue, nValues);
    CPDF_ColorStateData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("SetStrokePattern GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
...
{
    PDFIUM_GS_LOG("SetFont this=0x%08x font=0x%08x", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pFont);
    ...
    if (pStateData) {
        ...
        PDFIUM_GS_LOG("SetFont GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
    }
}
...
{
    PDFIUM_GS_LOG("SetRenderIntent this=0x%08x", (unsigned int)(FX_UINTPTR)this);
    CPDF_GeneralStateData* pData = GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("SetRenderIntent GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
    pData->m_RenderIntent = RI_StringToId(ri);
}
...
{
    PDFIUM_GS_LOG("SetLineDash this=0x%08x array=0x%08x phase=%d scale=%d", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pArray, (int)phase, (int)scale);
    CFX_GraphStateData* pData = m_GraphState.GetModify();
    if (!pData) {
        PDFIUM_GS_LOG("SetLineDash GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
    pData->m_DashPhase = FXSYS_Mul(phase, scale);
    pData->SetDashCount(pArray->GetCount());
    if (pArray->GetCount() && !pData->m_DashArray) {
        return;
    }
...
{
    PDFIUM_GS_LOG("ProcessExtGS enter this=0x%08x gs=0x%08x parser=0x%08x", (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pGS, (unsigned int)(FX_UINTPTR)pParser);
    CPDF_GeneralStateData* pGeneralState = m_GeneralState.GetModify();
    if (!pGeneralState) {
        PDFIUM_GS_LOG("ProcessExtGS GeneralState.GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
        return;
    }
    ...
    while (pos) {
        ...
        PDFIUM_GS_LOG("ProcessExtGS key=0x%08x this=0x%08x obj=0x%08x", key, (unsigned int)(FX_UINTPTR)this, (unsigned int)(FX_UINTPTR)pObject);
        switch (key) {
            case FXBSTR_ID('L', 'W', 0, 0):
                if (CFX_GraphStateData* pData = m_GraphState.GetModify()) {
                    pData->m_LineWidth = pObject->GetNumber();
                } else {
                    PDFIUM_GS_LOG("ProcessExtGS LW GraphState.GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
                }
                break;
            case FXBSTR_ID('L', 'C', 0, 0):
                if (CFX_GraphStateData* pData = m_GraphState.GetModify()) {
                    pData->m_LineCap = (CFX_GraphStateData::LineCap)pObject->GetInteger();
                } else {
                    PDFIUM_GS_LOG("ProcessExtGS LC GraphState.GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
                }
                break;
            case FXBSTR_ID('L', 'J', 0, 0):
                if (CFX_GraphStateData* pData = m_GraphState.GetModify()) {
                    pData->m_LineJoin = (CFX_GraphStateData::LineJoin)pObject->GetInteger();
                } else {
                    PDFIUM_GS_LOG("ProcessExtGS LJ GraphState.GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
                }
                break;
            case FXBSTR_ID('M', 'L', 0, 0):
                if (CFX_GraphStateData* pData = m_GraphState.GetModify()) {
                    pData->m_MiterLimit = pObject->GetNumber();
                } else {
                    PDFIUM_GS_LOG("ProcessExtGS ML GraphState.GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
                }
                break;
            ...
            case FXBSTR_ID('F', 'o', 'n', 't'):	{
                    ...
                    CPDF_TextStateData* pTextData = m_TextState.GetModify();
                    if (!pTextData) {
                        PDFIUM_GS_LOG("ProcessExtGS Font TextState.GetModify failed this=0x%08x", (unsigned int)(FX_UINTPTR)this);
                        break;
                    }
                    pTextData->m_FontSize = pFont->GetNumber(1);
...
{
    void* pData = m_Marks.AddSpace();
    if (!pData) {
        return;
    }
    CPDF_ContentMarkItem& item = *(CPDF_ContentMarkItem*)new (pData) CPDF_ContentMarkItem();
    item.SetName(name);
    if (pDict == NULL) {
        return;
    }
