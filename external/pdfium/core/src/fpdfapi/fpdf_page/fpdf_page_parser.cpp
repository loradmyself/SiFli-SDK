...
void CPDF_StreamContentParser::Handle_BeginMarkedContent_Dictionary()
{
    ...
    CPDF_ContentMarkData* pMarkData = m_CurContentMark.GetModify();
    if (!pMarkData) {
        return;
    }
    pMarkData->AddMark(tag, (CPDF_Dictionary*)pProperty, bDirect);
}
void CPDF_StreamContentParser::Handle_BeginMarkedContent()
{
    ...
    pMarkData->AddMark(tag, NULL, FALSE);
}
struct _FX_BSTR {
    FX_LPCSTR	m_Ptr;
    int			m_Size;
...
void CPDF_StreamContentParser::Handle_SetColorSpace_Fill()
{
    ...
    CPDF_ColorStateData* pData = m_pCurStates->m_ColorState.GetModify();
    if (!pData) {
        return;
    }
    pData->m_FillColor.SetColorSpace(pCS);
}
void CPDF_StreamContentParser::Handle_SetColorSpace_Stroke()
{
    ...
    pData->m_StrokeColor.SetColorSpace(pCS);
}
...
{
#if !defined(_FPDFAPI_MINI_) || defined(_FXCORE_FEATURE_ALL_)
    CPDF_GeneralStateData* pData = m_pCurStates->m_GeneralState.GetModify();
    if (!pData) {
        return;
    }
    pData->m_Flatness = GetNumber(0);
#endif
}
...
{
    CPDF_GraphStateData* pData = m_pCurStates->m_GraphState.GetModify();
    if (!pData) {
        return;
    }
    pData->m_LineJoin = (CFX_GraphStateData::LineJoin)GetInteger(0);
}
void CPDF_StreamContentParser::Handle_SetLineCap()
{
    ...
    pData->m_LineCap = (CFX_GraphStateData::LineCap)GetInteger(0);
}
...
void CPDF_StreamContentParser::Handle_SetMiterLimit()
{
    ...
    pData->m_MiterLimit = GetNumber(0);
}
...
{
    CPDF_TextStateData* pData = m_pCurStates->m_TextState.GetModify();
    if (!pData) {
        return;
    }
    pData->m_CharSpace = GetNumber(0);
}
...
void CPDF_StreamContentParser::AddTextObject(CFX_ByteString* pStrs, FX_FLOAT fInitKerning, FX_FLOAT* pKerning, int nsegs)
{
    ...
    if (textmode && textmode != 3 && textmode != 4 && textmode != 7) {
        CPDF_TextStateData* pTextState = pText->m_TextState.GetModify();
        if (!pTextState) {
            pText->Release();
            m_pLastTextObject = NULL;
            return;
        }
        FX_FLOAT* pCTM = pTextState->m_CTM;
...
void CPDF_StreamContentParser::OnChangeTextMatrix()
{
    ...
    FX_FLOAT* pTextMatrix = pData->m_Matrix;
...
void CPDF_StreamContentParser::Handle_NextLineShowText_Space()
{
    ...
    pData->m_WordSpace = GetNumber(2);
    pData->m_CharSpace = GetNumber(1);
...
void CPDF_StreamContentParser::AddPathObject(int FillType, FX_BOOL bStroke)
{
    ...
    if (PathPointCount <= 1) {
        if (PathPointCount && PathClipType) {
            ...
            if (!pPathData || !pPathData->AppendRect(0, 0, 0, 0)) {
                return;
            }
    ...
    CPDF_Path Path;
    CFX_PathData* pPathData = Path.New();
    if (!pPathData || !pPathData->SetPointCount(PathPointCount)) {
        return;
    }
