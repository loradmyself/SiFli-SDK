// Copyright 2014 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#ifndef _FX_MEMORY_H_
#define _FX_MEMORY_H_
#ifndef _FX_SYSTEM_H_
#include "fx_system.h"
#endif
#define FX_Alloc(type, size)						(type*)FXMEM_DefaultCallocDebug((size), sizeof(type), 0, __FILE__, __LINE__)
#define FX_Realloc(type, ptr, size)					(type*)FXMEM_DefaultReallocDebug(ptr, sizeof(type) * (size), 0, __FILE__, __LINE__)
#define FX_Free(ptr)								FXMEM_DefaultFree(ptr, 0)
#define FX_AllocNL(type, size)						FX_Alloc(type, size)
#define FX_ReallocNL(type, ptr, size)				FX_Realloc(type, ptr, size)

#ifdef __cplusplus
extern "C" {
#endif
void*	FXMEM_DefaultAlloc(size_t byte_size, int flags);
void*	FXMEM_DefaultCalloc(size_t count, size_t byte_size, int flags);
void*	FXMEM_DefaultRealloc(void* pointer, size_t new_size, int flags);
void*	FXMEM_DefaultAllocDebug(size_t byte_size, int flags, const char *file, int line);
void*	FXMEM_DefaultCallocDebug(size_t count, size_t byte_size, int flags, const char *file, int line);
void*	FXMEM_DefaultReallocDebug(void* pointer, size_t new_size, int flags, const char *file, int line);
void	FXMEM_DefaultFree(void* pointer, int flags);
void	FXMEM_ClearOOM(void);
int		FXMEM_HasOOM(void);
void	FXMEM_GetOOMInfo(int *count, size_t *maxsize);
void	FXMEM_DumpLeaks(void);
#ifdef __cplusplus
}
#endif
#ifdef __cplusplus
struct FX_Nothrow_t {};
extern const FX_Nothrow_t FX_Nothrow;

#ifndef _MSC_VER
inline void* operator new(size_t, void* buf) throw()
{
    return buf;
}
inline void operator delete(void*, void*) throw()
{
}
inline void* operator new[](size_t, void* buf) throw()
{
    return buf;
}
inline void operator delete[](void*, void*) throw()
{
}
#endif
inline void* operator new(size_t size, const FX_Nothrow_t&) throw()
{
    return FXMEM_DefaultCallocDebug(1, size, 0, __FILE__, __LINE__);
}
inline void operator delete(void* p, const FX_Nothrow_t&) throw()
{
    FXMEM_DefaultFree(p, 0);
}
inline void* operator new[](size_t size, const FX_Nothrow_t&) throw()
{
    return FXMEM_DefaultCallocDebug(1, size, 0, __FILE__, __LINE__);
}
inline void operator delete[](void* p, const FX_Nothrow_t&) throw()
{
    FXMEM_DefaultFree(p, 0);
}
inline void* operator new(size_t size, FX_LPCSTR file, int line) throw()
{
    return FXMEM_DefaultCallocDebug(1, size, 0, file, line);
}
inline void operator delete(void* p, FX_LPCSTR, int) throw()
{
    FXMEM_DefaultFree(p, 0);
}
inline void* operator new[](size_t size, FX_LPCSTR file, int line) throw()
{
    return FXMEM_DefaultCallocDebug(1, size, 0, file, line);
}
inline void operator delete[](void* p, FX_LPCSTR, int) throw()
{
    FXMEM_DefaultFree(p, 0);
}

class CFX_Object
{
public:
    void*			operator new (size_t size, FX_LPCSTR file, int line) throw()
    {
        return FXMEM_DefaultCallocDebug(1, size, 0, file, line);
    }
    void			operator delete (void* p, FX_LPCSTR file, int line)
    {
    FXMEM_DefaultFree(p, 0);
    }
    void*			operator new (size_t size) throw()
    {
    return FXMEM_DefaultCallocDebug(1, size, 0, __FILE__, __LINE__);
    }
    void*			operator new (size_t size, const FX_Nothrow_t&) throw()
    {
    return FXMEM_DefaultCallocDebug(1, size, 0, __FILE__, __LINE__);
    }
    void			operator delete (void* p)
    {
    FXMEM_DefaultFree(p, 0);
    }
    void			operator delete (void* p, const FX_Nothrow_t&) throw()
    {
    FXMEM_DefaultFree(p, 0);
    }
    void*			operator new[] (size_t size, FX_LPCSTR file, int line) throw()
    {
    return FXMEM_DefaultCallocDebug(1, size, 0, file, line);
    }
    void			operator delete[] (void* p, FX_LPCSTR file, int line)
    {
    FXMEM_DefaultFree(p, 0);
    }
    void*			operator new[] (size_t size) throw()
    {
    return FXMEM_DefaultCallocDebug(1, size, 0, __FILE__, __LINE__);
    }
    void*			operator new[] (size_t size, const FX_Nothrow_t&) throw()
    {
    return FXMEM_DefaultCallocDebug(1, size, 0, __FILE__, __LINE__);
    }
    void			operator delete[] (void* p)
    {
    FXMEM_DefaultFree(p, 0);
    }
    void			operator delete[] (void* p, const FX_Nothrow_t&) throw()
    {
    FXMEM_DefaultFree(p, 0);
    }
    void*			operator new (size_t, void* buf)
    {
        return buf;
    }
    void			operator delete (void*, void*)							{}
};
#endif
#ifdef __cplusplus
#define FX_NEW new(__FILE__, __LINE__)
#define FX_NEW_VECTOR(Pointer, Class, Count) \
    { \
        Pointer = FX_Alloc(Class, Count); \
        if (Pointer) { \
            for (int i = 0; i < (Count); i ++) new (Pointer + i) Class; \
        } \
    }
#define FX_DELETE_VECTOR(Pointer, Class, Count) \
    { \
        for (int i = 0; i < (Count); i ++) Pointer[i].~Class(); \
        FX_Free(Pointer); \
    }
class CFX_DestructObject : public CFX_Object
{
public:

    virtual ~CFX_DestructObject() {}
};
class CFX_GrowOnlyPool : public CFX_Object
{
public:

    CFX_GrowOnlyPool(size_t trunk_size = 16384);

    ~CFX_GrowOnlyPool();

    void	SetTrunkSize(size_t trunk_size)
    {
        m_TrunkSize = trunk_size;
    }

    void*	AllocDebug(size_t size, FX_LPCSTR file, int line)
    {
        return Alloc(size);
    }

    void*	Alloc(size_t size);

    void*	ReallocDebug(void* p, size_t new_size, FX_LPCSTR file, int line)
    {
        return NULL;
    }

    void*	Realloc(void* p, size_t new_size)
    {
        return NULL;
    }

    void	Free(void*) {}

    void	FreeAll();
private:

    size_t	m_TrunkSize;

    void*	m_pFirstTrunk;
};
#endif
#endif
