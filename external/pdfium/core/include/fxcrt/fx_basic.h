...
public:
    ...
    inline void				AppendByte(FX_BYTE byte)
    {
        ...
        if (!m_pBuffer || m_AllocSize <= m_DataSize) {
            return;
        }
...
public:
    ...
    {
        void* pData = InsertSpaceAt(m_nSize, 1);
        if (!pData) {
            return;
        }
        new (pData) ObjectClass(data);
    }
    ...
    ObjectClass&	Add()
    {
        void* pData = InsertSpaceAt(m_nSize, 1);
        if (!pData) {
            static ObjectClass s_Dummy;
            return s_Dummy;
        }
        return *(ObjectClass*) new (pData) ObjectClass();
    }
    ...
    FX_INT32		Append(const CFX_ObjectArray& src, FX_INT32 nStart = 0, FX_INT32 nCount = -1)
    {
        ...
        nSize = m_nSize;
        if (!InsertSpaceAt(m_nSize, nCount)) {
            return 0;
        }
...
public:
    ...
    {
        ElementType* pData = (ElementType*)CFX_BaseSegmentedArray::Add();
        if (!pData) {
            return;
        }
        *pData = data;
...
public:
    ...
    FX_BOOL			SetAt(FX_BSTR key, void* value);

    void			RemoveKey(FX_BSTR key);

    int				GetCount() const;

    FX_BOOL			AddValue(FX_BSTR key, void* pValue);
private:
...
public:
    ...
    ObjClass*			GetModify()
    {
        if (m_pObject == NULL) {
            ...
            CountedObj* pNewObject = FX_NEW CountedObj(*pOldObject);
            if (!pNewObject) {
                return NULL;
            }
            pNewObject->m_RefCount = 1;
            pOldObject->m_RefCount --;
