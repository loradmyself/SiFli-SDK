...
#define PDF_INIT_LOG(fmt, ...)
#define PDF_VIEW_LOG(fmt, ...)
#if defined(SOLUTION)
#include "rtthread.h"
#define PDF_OOM_LOG(fmt, ...) rt_kprintf("pdfium: " fmt "\n", ##__VA_ARGS__)
#else
#define PDF_OOM_LOG(fmt, ...)
#endif
...
class CFontMapper : public IFX_FontMapper
{
public:
	CFontMapper();
	virtual ~CFontMapper();

	virtual FXFT_Face FindSubstFont(
							...
							CFX_SubstFont* pSubstFont			// [OUT] Subst font data
						);
...
{
	PDF_INIT_LOG("FPDF_InitLibrary enter hInstance=%p API5=%d os=%d platform=%d", hInstance,
#ifdef API5
	             1,
#else
	             0,
#endif
	             _FX_OS_, _FXM_PLATFORM_);
#ifdef API5
	CPDF_ModuleMgr::Create();
	g_pModuleMgr = CPDF_ModuleMgr::Get();
	PDF_INIT_LOG("API5 Create done moduleMgr=%p", g_pModuleMgr);
	 #if _FX_OS_ == _FX_WIN32_MOBILE_ || _FX_OS_ == _FX_LINUX_EMBEDDED_
	 	PDF_INIT_LOG("API5 calling InitEmbedded()");
	 ...
	 #endif
#else
	PDF_INIT_LOG("non-API5 init path");
	g_pCodecModule = CCodec_ModuleMgr::Create();
	PDF_INIT_LOG("codec module=%p", g_pCodecModule);
	
	CFX_GEModule::Create();
	PDF_INIT_LOG("ge module=%p", CFX_GEModule::Get());
	CFX_GEModule::Get()->SetCodecModule(g_pCodecModule);
	
	CPDF_ModuleMgr::Create();
	PDF_INIT_LOG("module mgr=%p", CPDF_ModuleMgr::Get());
...
DLLEXPORT FPDF_DOCUMENT STDCALL FPDF_LoadDocument(FPDF_STRING file_path, FPDF_BYTESTRING password)
{
	...
	if (!pParser) {
		PDF_OOM_LOG("%s alloc CPDF_Parser failed", __func__);
		return NULL;
	}
	pParser->SetPassword(password);

	FX_DWORD err_code = pParser->StartParse((FX_LPCSTR)file_path);
	if (err_code || FXMEM_HasOOM()) {
		if (FXMEM_HasOOM()) PDF_OOM_LOG("%s oom after StartParse", __func__);
		delete pParser;
...
DLLEXPORT FPDF_DOCUMENT STDCALL FPDF_LoadMemDocument(const void* data_buf, int size, FPDF_BYTESTRING password)
{
	...
	if (!pMemFile) {
		PDF_OOM_LOG("%s alloc CMemFile failed", __func__);
		delete pParser;
		return NULL;
	}
...
DLLEXPORT FPDF_DOCUMENT STDCALL FPDF_LoadCustomDocument(FPDF_FILEACCESS* pFileAccess, FPDF_BYTESTRING password)
{
	...
	if (!pFile) {
		PDF_OOM_LOG("%s alloc CPDF_CustomAccess failed", __func__);
		delete pParser;
		return NULL;
	}
...
//	CPDF_Parser* pParser = (CPDF_Parser*)document;
	...
	if (!pPage) {
		PDF_OOM_LOG("%s alloc CPDF_Page failed", __func__);
		return NULL;
	}
	pPage->Load(pDoc, pDict);
	pPage->ParseContent();
	if (FXMEM_HasOOM()) {
		PDF_OOM_LOG("%s oom after ParseContent", __func__);
		delete pPage;
		return NULL;
	}
						...
						int rotate, int flags)
{
	...
	if (!pContext) {
		PDF_OOM_LOG("%s alloc CRenderContext failed", __func__);
		return;
	}
						...
						int size_x, int size_y, int rotate, int flags)
{
	...
	if (!pContext->m_pDevice) {
		PDF_OOM_LOG("%s alloc CFX_SkiaDevice failed", __func__);
		delete pContext;
		pPage->RemovePrivateData((void*)1);
		return;
	}
...
#else
	void* pDeviceBuf = FXMEM_DefaultCallocDebug(1, sizeof(CFX_FxgeDevice), 0, __FILE__, __LINE__);
	if (!pDeviceBuf) {
		PDF_OOM_LOG("%s alloc CFX_FxgeDevice failed", __func__);
		delete pContext;
		pPage->RemovePrivateData((void*)1);
		return;
	}
	pContext->m_pDevice = new (pDeviceBuf) CFX_FxgeDevice;

	FX_BOOL bAttached = FALSE;
	if (flags & FPDF_REVERSE_BYTE_ORDER)
		bAttached = ((CFX_FxgeDevice*)pContext->m_pDevice)->Attach((CFX_DIBitmap*)bitmap,0,TRUE);
	else
		bAttached = ((CFX_FxgeDevice*)pContext->m_pDevice)->Attach((CFX_DIBitmap*)bitmap);
	if (!bAttached) {
		PDF_OOM_LOG("%s attach CFX_FxgeDevice failed", __func__);
		delete pContext;
		pPage->RemovePrivateData((void*)1);
		return;
	}
	...
	Func_RenderPage(pContext, page, start_x, start_y, size_x, size_y, rotate, flags,TRUE,NULL);
	if (FXMEM_HasOOM()) {
		PDF_OOM_LOG("%s oom after Func_RenderPage", __func__);
		delete pContext;
		pPage->RemovePrivateData((void*)1);
		return;
	}
	if (pContext->m_pRenderer) {
		int continue_loops = 0;
		PDF_VIEW_LOG("RenderPageBitmap enter page=%p renderer=%p status=%d size=%dx%d flags=0x%x",
				     page, pContext->m_pRenderer, pContext->m_pRenderer->GetStatus(),
				     size_x, size_y, flags);
		while (pContext->m_pRenderer &&
		       pContext->m_pRenderer->GetStatus() == CPDF_ProgressiveRenderer::ToBeContinued) {
			if (FXMEM_HasOOM()) {
				PDF_OOM_LOG("%s oom before Continue", __func__);
				break;
			}
			continue_loops++;
			pContext->m_pRenderer->Continue(NULL);
			if (FXMEM_HasOOM()) {
				PDF_OOM_LOG("%s oom after Continue", __func__);
				break;
			}
			PDF_VIEW_LOG("RenderPageBitmap continue page=%p loop=%d status=%d",
					 page, continue_loops,
					 pContext->m_pRenderer ? pContext->m_pRenderer->GetStatus() : -1);
		}
		PDF_VIEW_LOG("RenderPageBitmap exit page=%p renderer=%p loops=%d final_status=%d",
				     page, pContext->m_pRenderer, continue_loops,
				     pContext->m_pRenderer ? pContext->m_pRenderer->GetStatus() : -1);
	} else {
		PDF_VIEW_LOG("RenderPageBitmap no_renderer page=%p size=%dx%d flags=0x%x",
				     page, size_x, size_y, flags);
	}
...
DLLEXPORT FPDF_BITMAP STDCALL FPDFBitmap_Create(int width, int height, int alpha)
{
	...
	if (!pBitmap) {
		PDF_OOM_LOG("%s alloc CFX_DIBitmap failed", __func__);
		return NULL;
	}
	if (!pBitmap->Create(width, height, alpha ? FXDIB_Argb : FXDIB_Rgb32)) {
		PDF_OOM_LOG("%s CFX_DIBitmap::Create failed", __func__);
		delete pBitmap;
		return NULL;
	}
	return pBitmap;
...
DLLEXPORT FPDF_BITMAP STDCALL FPDFBitmap_CreateEx(int width, int height, int format, void* first_scan, int stride)
{
	...
	if (!pBitmap->Create(width, height, fx_format, (FX_LPBYTE)first_scan, stride)) {
		PDF_OOM_LOG("%s CFX_DIBitmap::Create failed", __func__);
		delete pBitmap;
		return NULL;
	}
...
//#ifdef _LICENSED_BUILD_
	...
	if (!pContext->m_pOptions) {
		PDF_OOM_LOG("%s alloc CPDF_RenderOptions failed", __func__);
		return;
	}
//	CPDF_RenderOptions options;
	...
	if (!pContext->m_pOptions->m_pOCContext) {
		PDF_OOM_LOG("%s alloc CPDF_OCContext failed", __func__);
		return;
	}
	...
	if (!pContext->m_pContext) {
		PDF_OOM_LOG("%s alloc CPDF_RenderContext failed", __func__);
		pContext->m_pDevice->RestoreState();
		return;
	}
	pContext->m_pContext->Create(pPage);
	if (FXMEM_HasOOM()) {
		PDF_OOM_LOG("%s oom after CPDF_RenderContext::Create", __func__);
		pContext->m_pDevice->RestoreState();
		return;
	}
	...
	if (flags & FPDF_ANNOT) {
		...
		if (!pContext->m_pAnnots) {
			PDF_OOM_LOG("%s alloc CPDF_AnnotList failed", __func__);
			pContext->m_pDevice->RestoreState();
			return;
		}
		...
		if (FXMEM_HasOOM()) {
			PDF_OOM_LOG("%s oom after DisplayAnnots", __func__);
			pContext->m_pDevice->RestoreState();
			return;
		}
	}
	...
	if (!pContext->m_pRenderer) {
		PDF_OOM_LOG("%s alloc CPDF_ProgressiveRenderer failed", __func__);
		pContext->m_pDevice->RestoreState();
		return;
	}
	pContext->m_pRenderer->Start(pContext->m_pContext, pContext->m_pDevice, pContext->m_pOptions, pause);
	if (FXMEM_HasOOM()) {
		PDF_OOM_LOG("%s oom after renderer Start", __func__);
	}
	if (bNeedToRestore)
	{
	  pContext->m_pDevice->RestoreState();
