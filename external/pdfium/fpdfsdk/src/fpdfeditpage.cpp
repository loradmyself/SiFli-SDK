...
{
	if(!pageObject) return 0;
	CPDF_PageObject* pPageObj = (CPDF_PageObject*)pageObject;

	switch (pPageObj->m_Type)
	{
	case PDFPAGE_TEXT:
		return FPDF_PAGEOBJ_TEXT;
	case PDFPAGE_PATH:
		return FPDF_PAGEOBJ_PATH;
	case PDFPAGE_IMAGE:
		return FPDF_PAGEOBJ_IMAGE;
	case PDFPAGE_SHADING:
		return FPDF_PAGEOBJ_SHADING;
	case PDFPAGE_FORM:
		return FPDF_PAGEOBJ_FORM;
	default:
		return 0;
	}
}
...
{
	if(!pageObject || !left || !top || !right || !bottom)
		return FALSE;
	CPDF_PageObject* pPageObj = (CPDF_PageObject*)pageObject;
	*left = pPageObj->m_Left;
	*top = pPageObj->m_Top;
	*right = pPageObj->m_Right;
	*bottom = pPageObj->m_Bottom;
	return TRUE;
}
