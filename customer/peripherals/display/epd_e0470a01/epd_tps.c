...
{
    rt_kprintf("[TPS] tps_init start, vcom=%dmV\n", vcom_voltage);
    ...
    rt_kprintf("[TPS] finding i2c bus: %s\n", EPD_TPS_I2C_DEVICE_NAME);
    p_i2c_bus = (struct rt_i2c_bus_device *)rt_device_find(EPD_TPS_I2C_DEVICE_NAME);

    if (p_i2c_bus)
    {
        rt_kprintf("[TPS] i2c bus found, opening...\n");
        rt_device_open((rt_device_t)p_i2c_bus, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_TX | RT_DEVICE_FLAG_INT_RX);
        rt_kprintf("[TPS] i2c bus opened\n");
    }
    else
    {
        rt_kprintf("[TPS] ERROR: i2c bus not found!\n");
        return;
    }

    rt_i2c_configure(p_i2c_bus, &configuration);
    rt_kprintf("[TPS] i2c configured\n");

    rt_kprintf("[TPS] configuring pins: WAKEUP=%d, PWRCOM=%d, PWRUP=%d\n",
               EPD_TPS_WAKEUP_PIN, EPD_TPS_PWRCOM_PIN, EPD_TPS_PWRUP_PIN);
    ...
    rt_kprintf("[TPS] WAKEUP=0, PWRCOM=0, PWRUP=0\n");
    rt_pin_write(EPD_TPS_WAKEUP_PIN, 0);
    rt_pin_write(EPD_TPS_PWRCOM_PIN, 0);
    rt_pin_write(EPD_TPS_PWRUP_PIN, 0);
    rt_thread_mdelay(10);

    rt_kprintf("[TPS] WAKEUP=1 (pull high)\n");
    rt_pin_write(EPD_TPS_WAKEUP_PIN, 1);
    rt_thread_mdelay(10);

    rt_kprintf("[TPS] setting power sequence...\n");
    tps_power_sequence_set();
    rt_kprintf("[TPS] setting VPOS/VNEG...\n");
    tps_vposvneg_set();
    rt_kprintf("[TPS] setting VCOM...\n");
    tps_vcom_set(vcom_voltage);

    rt_kprintf("[TPS] tps_init done, vcom=%dmV\n", vcom_voltage);
#ifdef EPD_TPS_GOOD_PIN
    rt_kprintf("[TPS] tps_good_pin: %d\n", rt_pin_read(EPD_TPS_GOOD_PIN));
#endif
    rt_kprintf("[TPS] WAKEUP=%d, PWRCOM=0, PWRUP=0\n", rt_pin_read(EPD_TPS_WAKEUP_PIN));
    rt_pin_write(EPD_TPS_PWRCOM_PIN, 0);
    rt_pin_write(EPD_TPS_PWRUP_PIN, 0);
}
